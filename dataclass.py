...
class PICConfig:
    ...
    def compute_scales(self):
        ...
        vth_SI = math.sqrt(Te_J / m_e_SI)
        omega_p_SI = math.sqrt(self.n0 * e_SI**2 / (eps0_SI * m_e_SI))
        lambdaD_SI = vth_SI / omega_p_SI
        ...
        print(f"[normalize] v_th={vth_SI:.3e} m/s, ω_p={omega_p_SI:.3e} 1/s, λ_D={lambdaD_SI:.3e} m")
        print(f"T_e = {self.Te:.2f} eV, n0 = {self.n0:.2e} 10^15 m^-3")
        print(f"To recover SI: multiply by n0*m_e*v_th^2 = {self.n0*n0_ref*m_e_SI*vth_SI**2:.3e} J per unit area")
        # print("--------------------------------------")
